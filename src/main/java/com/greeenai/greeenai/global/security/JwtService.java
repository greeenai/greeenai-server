...
public class JwtService {
	...
	@PostConstruct
	protected void init() {
		accessTokenSecretKey = Keys.hmacShaKeyFor(getTokenProperty(ACCESS_TOKEN).secret().getBytes());
		refreshTokenSecretKey = Keys.hmacShaKeyFor(getTokenProperty(REFRESH_TOKEN).secret().getBytes());
	}
	...
	public String generateToken(String tokenType, Long memberId) {
		Date now = new Date();
		Date expirationDate = new Date(now.getTime() + getTokenProperty(tokenType).expirationMilliTime());
		return Jwts.builder()
