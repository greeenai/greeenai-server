...
class MemberServiceTest {
    ...
    class WithdrawMemberTests {
        ...
        void withdrawMember_shouldDeleteCurrentMember() {
            // Given
            Member currentMember = Member.create(TEST_NAME, TEST_EMAIL, TEST_OAUTH_ID, OauthProvider.APPLE);
            ...
            // Then
            verify(memberRepository).delete(currentMember); // 회원 삭제 검증
            assertThatCode(() -> memberRepository.delete(currentMember)).doesNotThrowAnyException(); // 예외 발생 여부 검증
        }
    }
}
